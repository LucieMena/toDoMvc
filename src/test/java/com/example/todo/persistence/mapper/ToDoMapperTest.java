...
public class ToDoMapperTest {
    ...
    void testToDtoFromDomain() {
        ...
        ToDoDto dto = ToDoMapper.toDto(domain);

        assertNotNull(dto);
        assertEquals(3L, dto.getId());
        assertEquals("Task 3", dto.getTitle());
        assertTrue(dto.isCompleted());
        assertEquals(15, dto.getOrder());
    }
    ...
    void testToDomainFromDto() {
        ...
        ToDo domain = ToDoMapper.toDomainFromDto(dto);

        assertNotNull(domain);
        assertEquals(4L, domain.getId());
        assertEquals("Task 4", domain.getTitle());
        assertFalse(domain.isCompleted());
        assertEquals(20, domain.getOrder());
    }
}
